...
const getFoodById = async (req, res) => {
    ...
    try {
        ...
        if (food) return res.status(200).json({ food, messages: 'ok' });
    } catch (error) {
...
const postFood = async (req, res) => {
    ...
    if (errors.length) return res.status(404).json({ food, message: errors[0].message });
    if (existsFood.length) return res.status(404).json({ food: existsFood, message: 'food already exists' })
